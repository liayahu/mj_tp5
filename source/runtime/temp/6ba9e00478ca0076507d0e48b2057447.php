<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:110:"C:\Users\think\my\tool\phpstudy\PHPTutorial\WWW\guan.com\web/../source/application/super\view\article\edit.php";i:1615084751;s:105:"C:\Users\think\my\tool\phpstudy\PHPTutorial\WWW\guan.com\source\application\super\view\layouts\layout.php";i:1615080373;s:122:"C:\Users\think\my\tool\phpstudy\PHPTutorial\WWW\guan.com\source\application\super\view\layouts\_template\tpl_file_item.php";i:1615042314;s:121:"C:\Users\think\my\tool\phpstudy\PHPTutorial\WWW\guan.com\source\application\super\view\layouts\_template\file_library.php";i:1608705778;}*/ ?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <script>
        ...
        STORE_URL = '<?= isset($super_url) ? $super_url : '' ?>';
    </script>
</head>
            ...
            <div class="am-fr tpl-header-navbar">
                <ul>
                    ...
                    <li class="am-text-sm tpl-header-navbar-welcome">
                        <a href="<?= url('super.user/renew') ?>">欢迎你，<span><?= $super['super']['user_name'] ?></span>
                        </a>
                    </li>
                    <!-- 退出 -->
                    <li class="am-text-sm">
                        ...
                        <fieldset>
                            ...
                            <div class="am-form-group">
                                ...
                                <div class="am-u-sm-9 am-u-end">
                                    <input type="text" class="tpl-form-input" name="article[article_name]"
                                           value="<?= $model['article_name'] ?>" required>
                                </div>
                            </div>
                            <div class="am-form-group">
                                <label class="am-u-sm-3 am-u-lg-2 am-form-label form-require">文章分类 </label>
                                <div class="am-u-sm-9 am-u-end">
                                    <select name="article[art_category_id]" required
                                            data-am-selected="{searchBox: 1, btnSize: 'sm',  placeholder:'请选择文章分类'}">
                                        <option value=""></option>
                                        <?php if (isset($catgory)): foreach ($catgory as $first): ?>
                                            <option value="<?= $first['art_category_id'] ?>"
                                                <?= $model['art_category_id'] == $first['art_category_id'] ? 'selected' : '' ?>>
                                                <?= $first['name'] ?></option>
                                            <?php if (isset($first['child'])): foreach ($first['child'] as $two): ?>
                                                <option value="<?= $two['art_category_id'] ?>"
                                                    <?= $model['art_category_id'] == $two['art_category_id'] ? 'selected' : '' ?>>
                                                    　　<?= $two['name'] ?></option>
                                                <?php if (isset($two['child'])): foreach ($two['child'] as $three): ?>
                                                    <option value="<?= $three['art_category_id'] ?>"
                                                        <?= $model['art_category_id'] == $three['art_category_id'] ? 'selected' : '' ?>>
                                                        　　　<?= $three['name'] ?></option>
                                                <?php endforeach; endif; endforeach; endif; endforeach; endif; ?>
                                    </select>
                                    <small class="am-margin-left-xs">
                                        <a href="<?= url('article.category/add') ?>">去添加</a>
                                    </small>
                                </div>
                            </div>
                            <div class="am-form-group">
                                <label class="am-u-sm-3 am-u-lg-2 am-form-label form-require">文章图片 </label>
                                <div class="am-u-sm-9 am-u-end">
                                            ...
                                            <?php foreach ($model['image'] as $key => $item): ?>
                                                <div class="file-item">
                                                    ...
                                                    <input type="hidden" name="article[images][]"
                                                           value="<?= $item['image_id'] ?>">
                                                    <span>图片别名：</span><input type="text" name="article[image_name][]" value="<?= $item['image_name'] ?>">
                                                    <span>alt说明：</span><input type="text" name="article[image_alt][]" value="<?= $item['image_alt'] ?>">
                                    ...
                                    <div class="help-block am-margin-top-sm">
                                        <small>尺寸750x750像素以上，大小2M以下 (可拖拽图片调整显示顺序 )，图片别名修改时候不带后缀，比如xxx.png图修改时候只需填xxx</small>
                                    </div>
                                </div>
                            </div>

                            <div class="am-form-group">
                                <label class="am-u-sm-3 am-u-lg-2 am-form-label form-require">文章关键词 </label>
                                <div class="am-u-sm-9 am-u-end">
                                    <input type="text" class="tpl-form-input" name="article[keywords]"
                                           value="<?= $model['keywords'] ?>" required>
                                </div>
                            </div>
                            <div class="am-form-group">
                                <label class="am-u-sm-3 am-u-lg-2 am-form-label form-require">文章描述 </label>
                                <div class="am-u-sm-9 am-u-end">
                            ...
                            <div class="am-form-group">
                                ...
                                <div class="am-u-sm-9 am-u-end">
                                    <!-- 加载编辑器的容器 -->
                                    <textarea id="container" name="article[content]"><?= $model['content'] ?></textarea>
                                </div>
                            </div>
                            <div class="widget-head am-cf">
                            ...
                            <div class="am-form-group">
                                ...
                                <div class="am-u-sm-9 am-u-end">
                                    <label class="am-radio-inline">
                                        <input type="radio" name="article[article_status]" value="10" data-am-ucheck
                                            <?= $model['article_status']['value'] == 10 ? 'checked' : '' ?> >
                                        显示
                                    </label>
                                    <label class="am-radio-inline">
                                        <input type="radio" name="article[article_status]" value="20" data-am-ucheck
                                            <?= $model['article_status']['value'] == 20 ? 'checked' : '' ?> >
                                        隐藏
                                    </label>
                                </div>
                            </div>
                            <div class="am-form-group">
                                <label class="am-u-sm-3 am-u-lg-2 am-form-label form-require">是否展示首页 </label>
                                <div class="am-u-sm-9 am-u-end">
                                    <label class="am-radio-inline">
                                        <input type="radio" name="article[is_home]" value="10" data-am-ucheck
                                            <?= $model['is_home']['value'] == 10 ? 'checked' : '' ?> >
                                        否
                                    </label>
                                    <label class="am-radio-inline">
                                        <input type="radio" name="article[is_home]" value="20" data-am-ucheck
                                            <?= $model['is_home']['value'] == 20 ? 'checked' : '' ?> >
                                        是
                                    </label>
                                </div>
                            </div>
...
<script src="/assets/store/js/app.js?t=7"></script>
