...
class Controller extends \think\Controller
{
    ...
    {
        // 商家登录信息
        $this->store = Session::get('hishop_store');
        // 当前路由信息
        $this->getRouteinfo();
        // 验证登录
        $this->checkLogin();
    ...
    private function layout()
    {
        ...
        if (!in_array($this->routeUri, $this->notLayoutAction)) {
            ...
            $this->assign([
                ...
                'setting' => Setting::getAll() ?: null,        // 当前官网设置
            ]);
        }
    }
    ...
    //更新网站地图
    public function updateSiteMap($url=''){
        $site=file_get_contents('./sitemap.xml');
        preg_match( '/<lastmod[^>]*([\s\S]*?)<\/lastmod>/i', $site, $matches );
        if(!empty($matches[0])){
            $site=preg_replace ('/<lastmod[^>]*([\s\S]*?)<\/lastmod>/i','<lastmod>'.date('Y-m-d',strtotime("-1 day")).'</lastmod>',$site);
            if(!empty($url)){
            $data='<url>
 <loc>'.$url.'</loc>
...
$data.='
            ...
            $site=preg_replace ('/<\/urlset>/i',$data,$site);
            }
            file_put_contents('./sitemap.xml',$site);
        }
    }
    /**
     * 验证登录状态
