 ...
 * @return string
 */
function hishop_hash($password)
{
    return md5(md5($password) . 'hishop_salt_SmTRx');
}
...
{
    $request = Request::instance();
    return $request->scheme() . '://' . $request->host() . $request->url();
}
