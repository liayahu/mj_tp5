...
//EOF;
        ...
        $this->doLogs($data);
        // 订单信息
        $payorder = $PayOrderModel->payDetail($data['out_trade_no']);
        empty($payorder) && $this->returnCode(true, '订单不存在');
        // 小程序配置信息
        $wxConfig = SettingModel::getItem('wxapp');
        // 设置支付秘钥
        ...
        if (($sign === $dataSign)
            ...
            // 更新订单状态
            $payorder->updatePayStatus($data['transaction_id']);
            // 发送短信通知
            $this->sendSms($payorder['order_no']);
            // 返回状态
     ...
     */
    private function sendSms($order_no)
    {
        // 短信配置信息
        $config = SettingModel::getItem('sms');
